...
if metric != "Top Speed (kph)":
    ...
    # Find the most recent past week with training data (not just immediate previous)
    ...
    if prev_training.empty:
        previous_week_total = 0
    else:
        last_non_empty_week = prev_training["Date"].dt.isocalendar().week.max()
        last_non_empty_year = prev_training["Date"].dt.isocalendar().year.max()
        previous_week_data = prev_training[
            (prev_training["Date"].dt.isocalendar().week == last_non_empty_week) &
            (prev_training["Date"].dt.isocalendar().year == last_non_empty_year)
        ]
